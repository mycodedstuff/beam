  ...
  let finish x = pure (Right x)
      ...
      step (PgRunReturning CursorBatching
        ...
        do query <- pgRenderSyntax conn syntax
           ...
           case mExtime of
            Just extime -> dbg (decodeUtf8 query <> " Executed in: " <> T.pack (show ((sec extime * 1000) + (nsec extime `div` 1000000))) <> " ms ")
            Nothing -> dbg (decodeUtf8 query)
           return res
...
data PgF next where
    ...
    PgLiftWithHandle :: ((Text -> IO ()) -> Pg.Connection -> IO a) -> (a -> next) -> PgF next
